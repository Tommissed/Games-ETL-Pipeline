...
from sqlalchemy import (
    Table, Column, Integer, Text, Date, Boolean, Numeric, Float,
...
def raw_games(context: OpExecutionContext, config: RAWGApiConfig) -> list[dict]:
    ...
    while non_empty_pages < config.max_pages:
        ...
        if not results:
            context.log.info(f"GAMES: No games found for partition {context.partition_key}, stopping fetch.")
            break
            ...
            context.log.info(f"GAMES: Fetched page {page}, total games: {total_fetched}")
    ...
    context.log.info(f"GAMES: Finished fetching RAWG data, total games: {total_fetched}")
    return games
...
def transformed_games(context: OpExecutionContext, raw_games: list[dict]) -> list[dict]:
    ...
    if not raw_games:
        context.log.info("GAMES: No raw games data for this partition. Returning empty list.")
        return []
    ...
    expected_cols = {
        ...
        "playtime", "suggestions_count", "updated", "user_game", "reviews_count",
        "community_rating", "saturated_color", "dominant_color", "platforms",
        "parent_platform", "genres", "stores", "clip", "tags", "esrb_rating",
        "short_screenshots"
    }
    ...
    df_selected = df_renamed[["game_id",
        "slug",
        "name",
        "released",
        "tba",
...
def raw_genres(context: OpExecutionContext, config: RAWGApiConfig) -> list[dict]:
    ...
    while True:
        context.log.info(f"GENRES: Fetching genres")
        ...
        if not results:
            context.log.info(f"GENRES: No genres found, stopping fetch.")
            break

        elif results:
            genres.extend(results)
            total_fetched += len(results)
            context.log.info(f"GENRES: Fetched page {page}, total genres: {total_fetched}")
    ...
    context.log.info(f"GENRES: Finished fetching RAWG data, total genres: {total_fetched}")
    return genres
...
def transformed_genres(context: OpExecutionContext, raw_genres: list[dict]) -> list[dict]:
    """
    ...
    if not raw_genres:
        context.log.info("GENRES: No raw genres data for this partition. Returning empty list.")
        return []

    df = pd.json_normalize(raw_genres) #using pandas, we normalize the list of dicts into a flat table
    ...
    if missing:
        context.log.warning(
            f"GENRES: Missing expected RAWG fields: {missing}"
        )
    ...
    df_selected = df_renamed[["genre_id", "name", "slug", "games_count", "image_background", "games"]]
...
def genres(context: OpExecutionContext, postgres_conn: PostgresqlDatabaseResource, transformed_genres=dict) -> None:
    """
    ...
    upsert_to_database(postgres_conn = postgres_conn, data=transformed_genres, table=genres, metadata=metadata)
...
def raw_platforms(context: OpExecutionContext, config: RAWGApiConfig) -> list[dict]:
    ...
    while True:
        context.log.info(f"PLATFORMS: Fetching platforms")
        ...
        if not results:
            context.log.info(f"PLATFORMS: No platforms found, stopping fetch.")
            break

        elif results:
            platforms.extend(results)
            total_fetched += len(results)
            context.log.info(f"PLATFORMS: Fetched page {page}, total platforms: {total_fetched}")
    ...
    context.log.info(f"PLATFORMS: Finished fetching RAWG data, total platforms: {total_fetched}")
    return platforms
...
def transformed_platforms(context: OpExecutionContext, raw_platforms: list[dict]) -> list[dict]:
    """
    ...
    if not raw_platforms:
        context.log.info("PLATFORMS: No raw platforms data for this partition. Returning empty list.")
        return []

    df = pd.json_normalize(raw_platforms) #using pandas, we normalize the list of dicts into a flat table

    expected_cols = {
        "id", "name", 'slug', 'games_count', 'image_background', 'image',
        "year_start", "year_end", "games"
    }
    ...
    if missing:
        context.log.warning(
            f"PLATFORMS: Missing expected RAWG fields: {missing}"
        )

    
    if df.empty:
        context.log.info("PLATFORMS: Normalized dataframe is empty. Returning empty list.")
        return []
...
def platforms(context: OpExecutionContext, postgres_conn: PostgresqlDatabaseResource, transformed_platforms=dict) -> None:
    """
    ...
    if not transformed_platforms:
        context.log.info("PLATFORMS: No transformed platforms to load. Skipping insert.")
        return
    ...
    upsert_to_database(postgres_conn = postgres_conn, data=transformed_platforms, table=platforms, metadata=metadata)
...
def raw_stores(context: OpExecutionContext, config: RAWGApiConfig) -> list[dict]:
    ...
    while True:
        context.log.info(f"STORES: Fetching stores")
        ...
        if not results:
            context.log.info(f"STORES: No stores found, stopping fetch.")
            break

        elif results:
            stores.extend(results)
            total_fetched += len(results)
            context.log.info(f"STORES: Fetched page {page}, total stores: {total_fetched}")
    ...
    context.log.info(f"STORES: Finished fetching RAWG data, total stores: {total_fetched}")
    return stores
...
def transformed_stores(context: OpExecutionContext, raw_stores: list[dict]) -> list[dict]:
    """
    ...
    if not raw_stores:
        context.log.info("STORES: No raw stores data for this partition. Returning empty list.")
        return []

    df = pd.json_normalize(raw_stores) #using pandas, we normalize the list of dicts into a flat table
    ...
    if missing:
        context.log.warning(
            f"STORES: Missing expected RAWG fields: {missing}"
        )
...
def stores(context: OpExecutionContext, postgres_conn: PostgresqlDatabaseResource, transformed_stores=dict) -> None:
    """
    ...
    upsert_to_database(postgres_conn = postgres_conn, data=transformed_stores, table=stores, metadata=metadata)
...
def raw_tags(context: OpExecutionContext, config: RAWGApiConfig) -> list[dict]:
    ...
    while non_empty_pages < config.max_pages:
        context.log.info(f"TAGS: Fetching tags")
        
        data = fetch_tags_page(api_key=config.api_key, page=page)
        results = data.get("results", [])

        if not results:
            context.log.info(f"TAGS: No tags found, stopping fetch.")
            break
...
def transformed_tags(context: OpExecutionContext, raw_tags: list[dict]) -> list[dict]:
    ...
    if not raw_tags:
        context.log.info("TAGS: No raw tags data for this partition. Returning empty list.")
        return []

    df = pd.json_normalize(raw_tags) #using pandas, we normalize the list of dicts into a flat table
    ...
    if missing:
        context.log.warning(
            f"TAGS: Missing expected RAWG fields: {missing}"
        )
...
def tags(context: OpExecutionContext, postgres_conn: PostgresqlDatabaseResource, transformed_tags=dict) -> None:
    """
    ...
    upsert_to_database(postgres_conn = postgres_conn, data=transformed_tags, table=tags, metadata=metadata)
